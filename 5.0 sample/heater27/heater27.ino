...
const int  g_temperature[8] = { 0, 20, 21, 22, 24, 26, 29, 35 };  
...
#define  File_MaxCnt  200 

String FileNames[File_MaxCnt];
int File_Cnt;
...
void branchFunc()
{
   String whButton;
    ...
    if( whButton.compareTo("Delete") == 0 ) 
        deleteFunc();
...
void handleRoot()
{
  ...
  content += "</td></tr><tr><td align='center'> Device : " + String(DeviceID) + ", Current IP Address : "+ String( Info.W.IPA ) + "</td></tr></table>";    
  ...
  int i;
   ...
   if( File_Cnt == 0 ) {
      Serial.println("No file!");     
		   ...
		   {		   
					if( server.arg("H"+String(i)).toInt() == 1 ) {	
  ...
  webPageBody += "<table cellpadding='2'cellspacing='2'class='T0'>";
    ...
    while (dir.next()) {
        Serial.print(dir.fileName());
                ...
                if( File_Cnt < File_MaxCnt ) {
                  FileNames[File_Cnt] = String(dir.fileName());
                  Serial.print("  FILE: ");
                  Serial.println(dir.fileName());
                  webPageBody += "<tr><td  style='width:30%'></td> <td style='width:40% text-align:left'> <input type='checkbox'name='H"+ String(File_Cnt) +"'value='1'><a href='" + String(dir.fileName()) + "'>" + String(dir.fileName()) +"</td> <td  style='width:30%'></td></tr>";   
                  server.serveStatic(FileNames[File_Cnt].c_str(),  SPIFFS, FileNames[File_Cnt].c_str() );
                  File_Cnt ++;
                }
   ...
   webPageBody += "<tr><td  style='width:30%'></td> <td  style='width:40% align:center'><input class='MB1'style='font-size:80%'type='submit'name='SMT'value='Delete'></td><td  style='width:30%'></td></tr>"; 
   webPageBody += "</table></form></div></body></html>"; 

   //Serial.println(webPageBody);
}
...
void writeFile( String path, const char * message)
{
...
void deleteFile(String path)
{
    ...
    if(SPIFFS.remove(path.c_str()) == true ){
...
void MakeLogFile(const char * message)
{
   time_t t;
   char path[100];
    ...
    t = now();
    memset(path, 0, 100);
    sprintf(path,"/F%02d%02d%02d-%02d%02d%02d.txt", year(t), month(t), day(t), hour(t),minute(t),second(t));

    writeFile( String(path), message  );
